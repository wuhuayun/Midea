    ...
    public class AscmWebService : YnWS.YnWebService
    {
        ...
        public string GetSupplierDriverDeliveryBatchSumMain(string ticket, int driverId, ref string message)
        {
            try
            {
                ...
                AscmDeliBatSumMain ascmDeliBatSumMain = AscmDeliBatSumMainService.GetInstance().GetByDriverId(driverId);
        ...
        public string TryGetAscmUser2(string userId, string userPwd, ref string message)
        {
            try
            {
                ...
                userPwd = BitConverter.ToString(output).Replace("-", "");
        ...
        [WebMethod()]
        public string GetKeeperCurrentJobPreparationMainList(string ticket, string userId, ref string message)
        {
            try
            {
                message = "";
                ...
                List<AscmWmsPreparationMain> listAscmWmsPreparationMain = AscmWmsPreparationMainService.GetInstance().GetCurrentJobList(userId);
                return YnBaseClass2.Helper.ObjectHelper.Serialize(listAscmWmsPreparationMain);
            }
            catch (Exception ex)
            {
        ...
        [WebMethod()]
        public string GetKeeperCurrentRequirePreparationMainList(string ticket, string userId, ref string message)
        {
            try
            {
                message = "";
                ...
                List<AscmWmsPreparationMain> listAscmWmsPreparationMain = AscmWmsPreparationMainService.GetInstance().GetCurrentRequireList(userId);
                return YnBaseClass2.Helper.ObjectHelper.Serialize(listAscmWmsPreparationMain);
            }
            catch (Exception ex)
            {
        ...
        public string GetKeeperCurrentWmsPreparationAllDetail(string ticket, int mainId, ref string message)
        {
            try
            {
                ...
                AscmWmsPreparationAllDetail ascmWmsPreparationAllDetail = AscmWmsPreparationMainService.GetInstance().GetAscmWmsPreparationAllDetail(mainId);
                return YnBaseClass2.Helper.ObjectHelper.Serialize(ascmWmsPreparationAllDetail);
            }
            catch (Exception ex)
            {
                message = ex.Message;
        ...
        public void SaveKeeperCurrentWmsPreparationAllDetail(string ticket, string ascmWmsPreparationAllDetail, ref string message)
        {
            try
            {
                ...
                //if (!IsTicketValid(ticket))
                //    throw new Exception("票证验证失败！");
                if (string.IsNullOrEmpty(ascmWmsPreparationAllDetail))
                    throw new Exception("备料单绑定信息不能为NULL或空");

                AscmWmsPreparationAllDetail _ascmWmsPreparationAllDetail =
                    (AscmWmsPreparationAllDetail)YnBaseClass2.Helper.ObjectHelper.Deserialize(typeof(AscmWmsPreparationAllDetail), ascmWmsPreparationAllDetail);

                if (_ascmWmsPreparationAllDetail == null)
                    throw new Exception("备料单绑定信息格式错误，序列化失败");

                AscmWmsPreparationMainService.GetInstance().SaveAscmWmsPreparationAllDetail(_ascmWmsPreparationAllDetail);
            }
            catch (Exception ex)
            {
                message = ex.Message;
                ...
                if (ilist != null && ilist.Count > 0)
                {
                    ...
                    foreach (AscmGetMaterialTask ascmGetMaterialTask in list)
                    {
                        if (ascmGetMaterialTask.status == "NOTEXECUTE")
                        {
                            ...
                            ascmGetMaterialTask.modifyUser = name;
        ...
        [WebMethod()]
        public bool EndExcuteTask(string ticket, string name, string releaseHeaderIds, ref string message)
        {
            List<AscmGetMaterialTask> list = null;
            try
            {
                ...
                {
                    //判断需求数量是否与领料数量一致
                    foreach (AscmGetMaterialTask ascmGetMaterialTask in ilist)
                    {
                        ...
                        if (taskWord == "T" && warehousePlace != "B631" && warehousePlace != "B632" && warehousePlace != "E222" && warehousePlace != "E221")
                        {
                            string sSql = "from AscmWipRequirementOperations where taskId = " + ascmGetMaterialTask.id;
                            IList<AscmWipRequirementOperations> ilistRequirementOperations = YnDaoHelper.GetInstance().nHibernateHelper.Find<AscmWipRequirementOperations>(sSql);
                            if (ilistRequirementOperations != null && ilistRequirementOperations.Count > 0)
                            {
                                foreach (AscmWipRequirementOperations ascmWipRequirementOperations in ilistRequirementOperations)
                                {
                                    ascmGetMaterialTask.totalRequiredQuantity += ascmWipRequirementOperations.requiredQuantity;
                                    ascmGetMaterialTask.totalGetMaterialQuantity += ascmWipRequirementOperations.getMaterialQuantity;
                                }
                            }
                        }
                    }
