...
{
    public partial class frmRelateDiscreteJobsView : DevComponents.DotNetBar.OfficeForm
    {
        private MonthCalendar monthCalendar;
        public int materialId { get; set; }
        
        public frmRelateDiscreteJobsView()
        {
            InitializeComponent();
        }
        ...
        private void frmRelateDiscreteJobsView_Load(object sender, EventArgs e)
        {
            StructureDataGridViewColumns(this.dgViewDiscreteJobsList);
        }
        ...
        private List<AscmDiscreteJobs> DataBindDataGridView(ref int count, string queryStartTime, string queryEndTime)
        {
            ...
            try
            {
                ...
                string jsonString = Service.GetRelatedDiscreteJobsList(frmMainView.encryptTicket, queryStartTime, queryEndTime, materialId, ref _ynPage, ref message);
        ...
        private void btnItemSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBoxEx.Show("查询排产单失败：请选择起止日期！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                if (materialId <= 0)
                {
                    MessageBoxEx.Show("查询排产单失败：请选择物料！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
        ...
        private int pagerControlJobs_EventPaging(CustomControl.EventPagingArg e)
        {
            ...
            if (dgViewDiscreteJobsList.DataSource != null)
                this.dgViewDiscreteJobsList.DataSource = null;

            this.dgViewDiscreteJobsList.AutoGenerateColumns = false;
            this.dgViewDiscreteJobsList.DataSource = pagerControlJobs.bindingSource;
            this.dgViewDiscreteJobsList.Refresh();
