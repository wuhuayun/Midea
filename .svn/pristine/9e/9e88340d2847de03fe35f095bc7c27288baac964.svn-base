...
<asp:Content ID="Content5" ContentPlaceHolderID="MainContent" runat="server">
	...
	<div region="center" title="" border="true" style="padding:0px;overflow:auto;">
		<table id="dataGridProfitAndLoss" title="盈亏登记单" style="" border="false" fit="true" singleSelect="true"
        ...
        <div id="tb1">
            ...
            <%} %>
            <a href="javascript:void(0);" class="easyui-linkbutton" plain="true" icon="icon-print" onclick="Print();">打印盈亏登记表</a>
            <% if (ynWebRight.rightEdit){ %>
            <a href="javascript:void(0);" class="easyui-linkbutton" plain="true" icon="icon-ok" onclick="">执行盈亏</a>
            <%} %>
        </div>
		<div id="editProfitAndLoss" class="easyui-window" title="盈亏登记单编辑" style="padding: 10px;width:640px;height:480px;"
			iconCls="icon-edit" closed="true" maximizable="false" minimizable="false" resizable="false" collapsible="false" modal="true" shadow="true">
			<div class="easyui-layout" fit="true">
				<div region="center" border="false" style="padding:10px;background:#fff;border:1px solid #ccc;">
                    <form id="editProfitAndLossForm" method="post" style="">
				...
				<div region="south" border="false" style="text-align:right;height:30px;line-height:30px;">
                    ...
                    <%} %>
					<a class="easyui-linkbutton" iconCls="icon-cancel" href="javascript:void(0)" onclick="$('#editProfitAndLoss').window('close');">取消</a>
				</div>
			</div>
		</div>
	</div>
    ...
    <script type="text/javascript">
        $(function () {
            $('#dataGridProfitAndLoss').datagrid({
                ...
                columns: [[
                    ...
                    { field: 'name', title: '货位', width: 80, align: 'center' },
                    { field: 'name', title: '原库存', width: 100, align: 'center' },
                    { field: 'name', title: '时间库存', width: 60, align: 'center' },
        ...
        function Save() {
            $.messager.confirm("确认", "确认提交保存?", function (r) {
                if (r) {
                    $('#editProfitAndLossForm').form('submit', {
                        url: '<%= Request.ApplicationPath=="/"?"":Request.ApplicationPath %>/' + currentId,
                        onSubmit: function () {
                            return $('#editProfitAndLossForm').form('validate');
                        },
                        success: function (r) {
                            var rVal = $.parseJSON(r);
                            if (rVal.result) {
                                $('#editProfitAndLoss').window('close');
                                currentId = selectRow.id;
                                Reload();
                            } else {
