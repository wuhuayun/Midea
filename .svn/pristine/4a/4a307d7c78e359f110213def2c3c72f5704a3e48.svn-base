...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
	...
	<div region="center" title="" border="true" style="padding:0px;overflow:auto;">
		<table id="dataGridReturnedMaterial" title="退料管理" style="" border="false" fit="true" singleSelect="true"
        ...
        <div id="tb1">
			...
			<a href="javascript:void(0);" class="easyui-linkbutton" plain="true" icon="icon-edit" onclick="Edit();">退料处理</a>
            <%} %>
        </div>
	</div>
</asp:Content>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentScriptAndCss" runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            $('#dataGridReturnedMaterial').datagrid({
                url: '',
                pagination: true,
                pageSize: 50,
                loadMsg: '更新数据......',
        ...
        function Reload() {
            $('#dataGridReturnedMaterial').datagrid('reload');
        }
        function Search() {
            var queryParams = $('#dataGridReturnedMaterial').datagrid('options').queryParams;
            queryParams.queryWord = $('#Search').val();
            $('#dataGridReturnedMaterial').datagrid('reload');
        }
        var currentId = null;
        function Edit() {
//            var selectRow = $('#dataGridReturnedMaterial').datagrid('getSelected');
//            if (selectRow) {
var id="123";
//id=selectRow.id;
                var url = '<%= Request.ApplicationPath=="/"?"":Request.ApplicationPath %>/Ascm/Warehouse/ReturnedMaterialEdit/' + id+"?mi=<%=Request["mi"].ToString() %>";
                parent.openTab('退料单_' + id, url);
