...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div region="center" title="" border="true" style="padding:0px;">
        <table id="dataGridMaterial" title="物料信息" style="" border="false" fit="true" checkonselect="false"
            selectoncheck="false" singleselect="true" idfield="id" sortname="id" sortorder="asc"
            striped="true" toolbar="#tbMaterial">
        </table>
        <div id="tbMaterial">
            <span>物料编码：</span><input id="startInventoryItemId" type="text" style="width: 100px;" />-<input
                id="endInventoryItemId" type="text" style="width: 100px;" />
            <a href="javascript:void(0);" class="easyui-linkbutton" plain="true" icon="icon-search"
                onclick="MaterialSearch();"></a>
            <% if (ynWebRight.rightVerify)
               { %>
            <a href="javascript:void(0);" class="easyui-linkbutton" plain="true" icon="icon-print"
                onclick="MaterialSelectedOk();">打印物料条码</a>
            <%} %>
            <% if (ynWebRight.rightAdd || ynWebRight.rightEdit)
               { %>
            <a href="javascript:void(0);" class="easyui-linkbutton" plain="true" icon="icon-add"
                onclick="ImportExcel();">导入</a> <a href="javascript:void(0);" class="easyui-linkbutton"
                    plain="true" icon="icon-print" onclick="ExportExcel();">模板导出</a>
            <% } %>
            ...
            <a href="javascript:void(0);" class="easyui-linkbutton" plain="true" icon="icon-cancel"
                onclick="Delete();">删除</a>
            <%} %>
            <iframe id="iframeExportToWord" name="iframeExportToWord" scrolling="auto" frameborder="0"
                src="" style="width: 0px; height: 0px; display: none;"></iframe>
        </div>
        <div id="popImport" class="easyui-window" title="数据导入" style="padding: 10px; width: 380px;
            height: 200px;" iconcls="icon-edit" closed="true" maximizable="false" minimizable="false"
            resizable="false" collapsible="false" modal="true" shadow="true">
            <div class="easyui-layout" fit="true">
                <div region="center" id="FileUpload" border="false" style="padding: 10px; background: #fff;
                    border: 1px solid #ccc;">
                    <form id="FormUpload" enctype="multipart/form-data" method="post">
                    <br />
                    <p>
                        <input type="file" id="fileImport" name="fileImport" size="35" value='' />&nbsp;&nbsp;</p>
                    </form>
                </div>
                <div region="south" border="false" style="text-align: right; height: 30px; line-height: 30px;">
                      <% if (ynWebRight.rightAdd || ynWebRight.rightEdit)
                       { %>
                    <a class="easyui-linkbutton" iconcls="icon-ok" href="javascript:void(0)" onclick="ImportOk()">
                        保存</a>  <%} %>
                         <a class="easyui-linkbutton" iconcls="icon-cancel" href="javascript:void(0)"
                            onclick="$('#popImport').window('close');">取消</a>
                </div>
            </div>
        </div>
        <div id="progressbarDiv" class="easyui-window" title="" style="padding: 12px; width: 460px;
            height: 80px;" closed="true" closable="false" maximizable="false" minimizable="false"
            resizable="false" collapsible="false" modal="true" shadow="true">
            <div id="p" class="easyui-progressbar" style="width: 400px;">
            </div>
        </div>
        </div>
        <div id="editDataGridMaterial" class="easyui-window" title="打印清单确认" style="padding: 10px;
            width: 610px; height: 400px;" iconcls="icon-edit" closed="true" maximizable="false"
            minimizable="false" resizable="false" collapsible="false" modal="true" shadow="true">
            <div class="easyui-layout" fit="true">
                <div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
                    <table id="dgEditDataGridMaterial" style="width: 100%;" class="easyui-datagrid" border="0"
                        border="false" data-options="fit: true,
                                                  rownumbers: true,
                                                  idField: 'id',
                                                  sortName: 'id',
                                                  sortOrder: 'asc',
                        ...
                        <thead>
                            <tr>
                                ...
                                <th data-options="field:'totalNumber',width:50,align:'center',editor:'numberbox'">
                                    总数量
                                </th>
                ...
                <div region="south" border="false" style="text-align: right; height: 30px; line-height: 30px;">
                    <a id="unitPrint" class="easyui-linkbutton" iconcls="icon-ok" href="javascript:void(0)"
                        onclick="MaterialPrint(true);">单元数打印</a> <a id="totalPrint" class="easyui-linkbutton"
                            iconcls="icon-ok" href="javascript:void(0)" onclick="MaterialPrint(false);">总数打印</a>
                    <a class="easyui-linkbutton" iconcls="icon-cancel" href="javascript:void(0)" onclick="$('#editDataGridMaterial').window('close');">
                        取消</a>
                </div>
            </div>
        </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentScriptAndCss" runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            $('#dataWhDepartmentPosition').datagrid({
