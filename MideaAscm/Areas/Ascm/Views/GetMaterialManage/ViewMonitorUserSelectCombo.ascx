...
<% selectCombo.id = string.IsNullOrEmpty(selectCombo.id) ? "MonitorUserSelect" : selectCombo.id; %>
...
<select id="<%=selectCombo.id %>" name="<%=selectCombo.id %>" class="easyui-combogrid" style="width:<%=selectCombo.width %>;"
    data-options="panelWidth: <%=selectCombo.panelWidth %>,
                  ...
                  <%} %>
                  url: '<%= Request.ApplicationPath=="/"?"":Request.ApplicationPath %>/Ascm/GetMaterialManage/MonitorUserAscxList',
                  columns: [[
