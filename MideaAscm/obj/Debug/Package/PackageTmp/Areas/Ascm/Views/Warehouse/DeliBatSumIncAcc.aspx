...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div region="west" title="" border="false" style="width:215px; padding:0px 2px 0px 0px; overflow:hidden;">
        <div class="easyui-panel" title="合单接收" fit="true" border="true" style="overflow:hidden;">
            <div class="easyui-layout" fit="true">
                <div region="north" border="false" style="overflow:hidden;" title="">
                    <div style="margin:5px;width:200px;">
                        <span>合单条码：</span>
                        <input type="text" id="deliBatSumBarcode" name="deliBatSumBarcode" style="width:130px;" value=""/>
                    </div>
		...
		<div class="easyui-panel" fit="true" border="false" style="overflow:hidden;">
			<div class="easyui-layout" fit="true" style="">
                ...
                <div region="center" border="false" fit="false" style="padding:0px 0px 2px 0px;overflow:hidden;">
                                ...
                                <table id="dgAssignWarelocation" title="货位信息" class="easyui-datagrid" style="" border="false"
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-options="field:'deliBatSumBarcode',width:80,align:'center'">批条码号</th>
                    ...
                    <%--<%Html.RenderPartial("~/Areas/Ascm/Views/Warehouse/DeliBatIncAccMaterialPrint.ascx"); %>--%>
                </div>
            </div>
        </div>
    </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentScriptAndCss" runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            $('#deliBatSumBarcode').keypress(function (e) {
            ...
            $('#dgDeliveryOrderBatch').datagrid({
                ...
                frozenColumns: [[
                    ...
                    { field: 'deliBatSumDocNumber', title: '合单号', width: 95, align: 'left' },
                    { field: 'deliBatSumBarcode', title: '合单条码', width: 65, align: 'center' },
                ...
                rowStyler: function (index, row) {
                    if (row.exeResult) {
                        return 'color:red;';
                    } else if (row.ascmStatusCn == '<%=AscmDeliveryOrderBatch.AscmStatusDefine.DisplayText(AscmDeliveryOrderBatch.AscmStatusDefine.receiveFail)%>') {
                        return 'color:red;';
                    } else if (row.ascmStatusCn == '<%=AscmDeliveryOrderBatch.AscmStatusDefine.DisplayText(AscmDeliveryOrderBatch.AscmStatusDefine.enterDoor)%>') {
                        return 'color:blue;';
                    } else if (row.checkQuantity > 0) {
                        return 'color:blue;';
                    }
                },
        ...
        function AddDeliBatToDetail() {
            if (editDeliOrderBatEditing()) {
                var deliBatSumBarcode = $('#deliBatSumBarcode').val();
                if (!isNotNullOrEmpty(deliBatSumBarcode)) {
                    $.messager.alert("提示", "请输入供应商合单条码号", "info", function () {
                        $('#deliBatSumBarcode').focus();
                    });
                    return;
                }
                $.ajax({
                    url: '<%= Request.ApplicationPath=="/"?"":Request.ApplicationPath %>/Ascm/Warehouse/DeliBatSumIncAccAdd/',
                    type: "post",
                    dataType: "json",
                    data: { "deliBatSumBarcode": deliBatSumBarcode },
                    success: function (r) {
                        sound.stop().play();
                        if (r.success) {
                            $.each(r.jsonResults, function (i, jr) {
                                if (jr.retCode == barcodeType.batchBarcode)
                                    AddAccExeResultMsg(jr.retCode, jr.key, jr.retMessage);
                                else {
                                    var rows = $('#dgDeliveryOrderBatch').datagrid('getRows');
                                    var eixt = false;
                                    $.each(rows, function (i, item) {
                                        if (item.id == jr.entity.id) {
                                            eixt = true;
                                            return;
                                        }
                                    });
                                    if (!eixt)
                                        $('#dgDeliveryOrderBatch').datagrid('appendRow', jr.entity);
                                }
                            });

                            //$('#deliBatSumBarcode').val("");
                            $('#deliBatSumBarcode').focus();
                        } else if (r.retCode == barcodeType.deliBatSum) {
                            AddAccExeResultMsg(r.retCode, r.key, r.retMessage);
                            //$('#deliBatSumBarcode').val("");
                            $('#deliBatSumBarcode').focus();
                        }
                    }
            ...
            $.each(checkRows, function (i, row) {
                $.each(r.entity, function (j, item) {
                    if (item.billId == row.id) {
                        AddAccExeResultMsg(barcodeType.batchBarcode, item.docNumber, item.returnMessage);
    ...
    <script type="text/javascript">
        var barcodeType = {
            ...
            getText: function (type) {
                if (type == this.deliBatSum) return "合单条码号";
                else if (type == this.batchBarcode) return "批条码号";
            }
        };
        function AddAccExeResultMsg(type, barCode, resultMsg) {
            var newResultMsg = "<li>"
                             + "<div>" + barcodeType.getText(type) + "：" + barCode + "</div>"
