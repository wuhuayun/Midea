...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div region="center" title="" border="true" style="padding: 0px; overflow: auto;">
        <table id="dgDiscreteJobs" title="排产单管理" style="" border="false" fit="true"
        ...
        <div id="tb1">
            ...
            <select id="queryType" name="queryType" style="width:80px;">
                ...
                <option value="dz">电装</option>
            </select>
            <span>排产员：</span>
            <input id="queryRanker" type="text" style="width: 100px;" />
            <%} %>
        ...
        <div id="addDiscreteJobs" class="easyui-window" title="上传排产单" style="padding: 10px;width:540px;height:420px;"
			iconCls="icon-edit" closed="true" maximizable="false" minimizable="false" resizable="false" collapsible="false" modal="true" shadow="true">
			<div class="easyui-layout" fit="true">
				<div region="center" border="false" style="padding:10px;background:#fff;border:1px solid #ccc;">
                    <form id="addDiscreteJobsForm" method="post" enctype="multipart/form-data" style="">
				        <table style="width:100%;" border="0" cellpadding="0" cellspacing="0">
                            <tr style="height:24px">
                                ...
                                <td>
                                    <input type="file" name="fileLoad" id="fileLoad" size="23" style="width:300px;" />
                                    <span style="color: Red;">*仅支持2003Excel</span>
					            </td>
				            </tr>
				        </table>
			        </form>
				...
				<div region="south" border="false" style="text-align:right;height:30px;line-height:30px;">
					...
					<a id="A1" class="easyui-linkbutton" iconCls="icon-ok" href="javascript:void(0)" onclick="Save()">保存</a>
                    <%} %>
					<a class="easyui-linkbutton" iconCls="icon-cancel" href="javascript:void(0)" onclick="$('#addDiscreteJobs').window('close');">取消</a>
				</div>
			</div>
		</div>
    ...
    <script type="text/javascript">
        $(function () {
            $('#dgDiscreteJobs').datagrid({
                ...
                singleSelect: false,
                loadMsg: '更新数据......',
                url: '<%= Request.ApplicationPath=="/"?"":Request.ApplicationPath %>/Ascm/GetMaterialManage/DiscreteJobsList/',
                frozenColumns: [[
        ...
        function Query() {
            var queryParams = $('#dgDiscreteJobs').datagrid('options').queryParams;
            queryParams.queryWord = $('#queryDate').datebox('getText');
            queryParams.queryType = $('#queryType').val();
            queryParams.queryRanker = $('#queryRanker').val();
            $('#dgDiscreteJobs').datagrid('reload');
        }
        var currentId = null;
        var EditOrAdd = null;
        function Add() {
            $('#addDiscreteJobs').window('open');
            $("#addDiscreteJobsForm")[0].reset();
        ...
        function Edit() {
            ...
            if (selectRow) {
                ...
                var sUrl = '<%= Request.ApplicationPath=="/"?"":Request.ApplicationPath %>/Ascm/GetMaterialManage/DiscreteJobsEdit/' + selectRow.id;
                $.ajax({
                    url: sUrl,
                    type: "post",
                    dataType: "json",
        ...
        function Save() {
            $.messager.confirm("确认", "确认提交保存?", function (r) {
                if (r) {
                    switch (EditOrAdd) {
                        case ("Add"):
                            $('#addDiscreteJobsForm').form('submit', {
                                url: '<%= Request.ApplicationPath=="/"?"":Request.ApplicationPath %>/Ascm/GetMaterialManage/DiscreteJobsSave/' + currentId,
                                onSubmit: function () {
                                    return $('#addDiscreteJobsForm').form('validate');
                                },
                                success: function (r) {
                                    var rVal = $.parseJSON(r);
                                    if (rVal.result) {
                                        $('#addDiscreteJobs').window('close');
                                        currentId = rVal.id;
                                        Query();
                                        if(rVal.message != "" && rVal.message != null) {
                                            var myArray = rVal.message.split(",");
                                            var str = "<div style='text-align:center;'><ul>";
                                            ...
                                            str += "</ul></div>";
                                            $.messager.alert('提示', '已存在的作业号共' + myArray.length + '个，具体为：'+ str, '');
                                        }
                                    } else {
                        ...
                        case ("Edit"):
                            $('#editDiscreteJobsForm').form('submit', {
                                url: '<%= Request.ApplicationPath=="/"?"":Request.ApplicationPath %>/Ascm/GetMaterialManage/DiscreteJobsEditSave/' + currentId,
                                onSubmit: function () {
    ...
    <script type="text/javascript">
        ...
        function ImportOk() {
            ...
            $('#FormUpload').form('submit', {
                url: '<%= Request.ApplicationPath=="/"?"":Request.ApplicationPath %>/Ascm/GetMaterialManage/DiscreteJobsImport/',
                onSubmit: function () {
